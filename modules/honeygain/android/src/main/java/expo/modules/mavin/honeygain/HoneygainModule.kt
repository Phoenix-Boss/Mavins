// modules/modules/honeygain/android/src/main/java/expo/modules/mavin/honeygain/HoneygainModule.kt
// ✅ COMPLETE FIXED - Expo SDK 52 + Official Honeygain API (2026)
...
class HoneygainModule(appContext: AppContext? = null) : Module(appContext) {
    companion object {
    ...
    override fun definition() = ModuleDefinition {
        ...
        AsyncFunction("setNotification") { 
            notificationId: Int,
            title: String,
            content: String,
            promise: Promise 
        ->
            setNotification(notificationId, title, content, promise)
        }
        ...
        Property("isBackground") { 
            ...
            set { value ->
                safeExecute { HgSdk.isBackground = value as? Boolean ?: false }
            }
        }
        ...
        Property("launchOnBoot") { 
            ...
            set { value ->
                safeExecute { HgSdk.launchOnBoot = value as? Boolean ?: false }
            }
        }
        ...
        Property("enableLogging") { 
            ...
            set { value ->
                safeExecute { HgSdk.enableLogging = value as? Boolean ?: false }
            }
        }
        ...
        Property("sdkInfo") {
            get {
                mapOf(
                    "name" to "Honeygain SDK",
                    "version" to "1.3.1",
                    "minAndroidVersion" to 21,
                    "minAndroidVersionName" to "Android 5.0 / Lollipop"
                )
            }
        }
    ...
    private fun initializeHoneygain() {
        ...
        val context = appContext?.androidContext ?: run {
            Log.w(TAG, "No Android context")
            return
        }
        ...
        try {
            ...
            // Configure Honeygain with optimal defaults
            safeExecute {
            ...
            Log.d(TAG, "✅ Honeygain module ready")
        } catch (e: Exception) {
    ...
    private fun initializeWithApiKey(apiKey: String, promise: Promise) {
        ...
        if (context == null) {
            promise.reject("NO_CONTEXT", "Android context not available")
            return
        }
    ...
    private fun setupErrorMonitoring() {
        safeExecute {
            HgSdk.onError = { error: Throwable ->
                ...
                // Auto-restart if opted in and not running
    ...
    private fun configureSdk(config: Map<String, Any>, promise: Promise) {
        try {
            config.forEach { (key, value) ->
                when (key) {
                    "isBackground" -> safeExecute { HgSdk.isBackground = value as? Boolean ?: false }
                    "launchOnBoot" -> safeExecute { HgSdk.launchOnBoot = value as? Boolean ?: false }
                    "enableLogging" -> safeExecute { HgSdk.enableLogging = value as? Boolean ?: false }
                }
            }
    ...
    private fun requestConsent(promise: Promise) {
        ...
        if (activity == null) {
            promise.reject("NO_ACTIVITY", "Activity context not available")
            return
        }
        ...
        try {
            activity.runOnUiThread {
                try {
                    HgSdk.requestConsent(activity)
                    
                    // Check consent result after dialog closes
                    mainHandler.postDelayed({
                        if (safeValue { HgSdk.isOptedIn } == true) {
                            sendEvent("onConsentGranted", mapOf(
                                "timestamp" to System.currentTimeMillis()
                    ...
                    promise.resolve(mapOf(
                        ...
                        "nextStep" to "Check isOptedIn property"
                    ))
                } catch (e: Exception) {
    ...
    private fun downloadPresetWithBandwidth(presetName: String, promise: Promise) {
        ...
        try {
            ...
            promise.resolve(mapOf(
                ...
                "message" to "Honeygain activated (Background + Boot enabled)",
                "balance" to balance,
    ...
    private fun clearLastError(promise: Promise) {
        try {
            // HgSdk doesn't have a direct clear method, but we can ignore it
            promise.resolve(mapOf(
                "success" to true,
    ...
    private fun setNotification(notificationId: Int, title: String, content: String, promise: Promise) {
        // This is a placeholder - actual Notification object creation requires more context
        promise.reject(
            "NOT_IMPLEMENTED", 
            "Custom notification requires Notification object creation in native code. " +
            "Use HgSdk.notification and HgSdk.notificationId directly."
        )
    }
