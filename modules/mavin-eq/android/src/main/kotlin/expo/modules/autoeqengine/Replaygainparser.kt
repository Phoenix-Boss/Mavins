 ...
 * Supports:
 *   ✅ ID3v2.3 / ID3v2.4  — TXXX frames (MP3, AAC-in-MP3 container)
 *   ✅ Vorbis Comment       — FLAC, OGG
 *   ✅ MP4 / M4A iTunes    — ----:com.apple.iTunes:replaygain_* free-form atoms
 *
 ...
 *   TRACK  — use track gain (best for shuffle / random playback)
 *   ALBUM  — use album gain (best for listening to full albums, preserves relative loudness)
 *   RADIO  — alias for TRACK (legacy name from original ReplayGain spec)
 *   OFF    — do not apply any gain (raw file level)
 *
 * Target loudness: –14 LUFS (EBU R128 streaming standard).
...
object ReplayGainParser {
     ...
     * This must NOT be called on the audio thread — call from a coroutine/background thread.
     */
    ...
    private fun parseMp4(file: File): ReplayGainInfo? {
        // Read first 512 KB — enough to contain all metadata atoms
    ...
    /** Parse "+3.21 dB" or "-6.54 dB" → Float. Returns null on parse failure. */
    ...
    /** Parse "0.998245" → Float. Returns null on parse failure. */
