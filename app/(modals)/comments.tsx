 ...
 *
 * Receives params:
 *   songId: string — YouTube video ID
 *   title:  string — Song title (display only)
 */
      ...
      <View style={ciStyles.row}>
        ...
        <View style={ciStyles.content}>
          <View style={ciStyles.authorRow}>
            <Text style={ciStyles.authorName} numberOfLines={1}>
              {comment.author.name}
            </Text>
          ...
          <View style={ciStyles.actionsRow}>
            <TouchableOpacity style={ciStyles.actionBtn} onPress={handleLike} hitSlop={8}>
              <Ionicons
                ...
                size={14}
                color={comment.isLikedByUser ? C.gold : C.textSub}
              />
              {comment.likeCount > 0 && (
                <Text style={[ciStyles.actionText, comment.isLikedByUser && ciStyles.actionTextActive]}>
                  {comment.likeCount}
                </Text>
              )}
            </TouchableOpacity>
            ...
            {hasReplies && !isNested && (
              ...
              >
                ...
                <Text style={[ciStyles.actionText, ciStyles.replyCountText]}>
...
const ciStyles = StyleSheet.create({
  ...
  uploaderBadge: {
    backgroundColor: C.goldFill, paddingHorizontal: 6, paddingVertical: 2,
    borderRadius: 4, borderWidth: 0.5, borderColor: C.borderGold,
  },
        ...
        <View style={rmStyles.sheet}>
          ...
          >
            {posting ? (
              ...
              <Text style={rmStyles.postBtnText}>Post Reply</Text>
            )}
          </TouchableOpacity>
        </View>
      </KeyboardAvoidingView>
...
const rmStyles = StyleSheet.create({
  ...
  handle: {
    ...
    backgroundColor: "rgba(255,255,255,0.15)", alignSelf: "center", marginTop: 10, marginBottom: 16,
  },
  ...
  input: {
    ...
    fontSize: 14, color: C.text, minHeight: 100, textAlignVertical: "top",
    borderWidth: 0.5, borderColor: C.border,
  },
...
export default function CommentsModal() {
  ...
  // Pagination using NativePage from MavinEngine
  ...
  const fetchComments = useCallback(async (page?: NativePage, isLoadMore = false) => {
    ...
    try {
      ...
      // Check if this is a CommentsPage (has commentsCount) or CommentsMorePage
      const isFirstPage = 'commentsCount' in result;
  ...
  // Like/unlike comment (local UI update only)
  ...
  // Post new top-level comment (optimistic update)
  const handlePostComment = useCallback(async () => {
    ...
    try {
      ...
      fetchComments(); // Refresh to revert optimistic update
    } finally {
      setPostingComment(false);
    }
  }, [newCommentText, fetchComments]);

  // Post reply (optimistic update)
...
const styles = StyleSheet.create({
  ...
  inputBar: {
    flexDirection: "row", alignItems: "flex-end", paddingHorizontal: 16, paddingTop: 10,
    borderTopWidth: 0.5, borderTopColor: C.borderGold, backgroundColor: C.bg, gap: 10,
  },
  input: {
    flex: 1, minHeight: 40, maxHeight: 120, backgroundColor: C.surfaceRaised, borderRadius: 20,
    borderWidth: 0.5, borderColor: C.border, paddingHorizontal: 14, paddingVertical: 10,
