...
// ============================================================================
// Equalizer UI — all DSP calls go through MavinPlayer's EqualizerProcessor.
//
// ── DSP architecture ─────────────────────────────────────────────────────────
// The full processing chain lives inside ExoPlayer's audio pipeline:
//   EqualizerProcessor → CompressorProcessor → CrossfeedProcessor →
//   ConvolutionProcessor → FxProcessor → PeakMeterProcessor
//
// This screen controls EqualizerProcessor exclusively:
//   • applyEQBands(gains: number[31])  — push 31-band gains to the DSP
//   • setEQEnabled(enabled: boolean)   — bypass / re-enable the processor
//
// All other processors (compressor, crossfeed, FX, etc.) are controlled from
// their own dedicated screens. No RNTP calls exist anywhere in this file.
// ─────────────────────────────────────────────────────────────────────────────

import React, { useState, useEffect, useCallback, useRef } from 'react';
import {
  Text, TouchableOpacity, View, StyleSheet, Dimensions,
  Modal, ScrollView, Platform, TextInput, Alert, FlatList,
} from 'react-native';
...
// MavinPlayer — all EQ DSP calls go through this module directly.
// getPlayerModule() returns the same instance that was booted in playerSetup.ts.
...
// 10 UI bands (frequency labels shown to the user)
const FREQUENCY_BANDS = [
  { label: '31',  bandIndex: 0 }, { label: '62',  bandIndex: 1 },
  { label: '125', bandIndex: 2 }, { label: '250', bandIndex: 3 },
  { label: '500', bandIndex: 4 }, { label: '1k',  bandIndex: 5 },
  { label: '2k',  bandIndex: 6 }, { label: '4k',  bandIndex: 7 },
  { label: '8k',  bandIndex: 8 }, { label: '16k', bandIndex: 9 },
];

// Maps each of the 10 UI sliders to a range of the 31 native EQ bands.
// EqualizerProcessor in the DSP chain operates on 31 bands.
const BAND_MAP: [number, number][] = [
  [0, 1], [2, 3], [4, 6], [7, 9], [10, 13],
  [14, 17], [18, 21], [22, 25], [26, 29], [30, 30],
];
...
interface EQState {
  enabled:        boolean;
  bands:          number[]; // 10 UI band values (-15 … +15 dB)
  bass:           number;   // knob 0–100
  treble:         number;   // knob 0–100
...
function sanitiseBands(raw: any[]): number[] {
  ...
  for (let i = 0; i < 10; i++) {
    const v = raw[i];
    out[i] = typeof v === 'number' && isFinite(v)
      ? Math.max(-15, Math.min(15, v)) : 0;
  }
  return out;
}

/** Expand 10 UI bands into the 31-band array EqualizerProcessor expects. */
...
export default function EqualizerScreen() {
  ...
  // EQ is only active on Android (where MavinPlayer is available)
  const isActive = eqState.enabled && Platform.OS === 'android';

  // ── DSP helpers ────────────────────────────────────────────────────────────

  /** Push all 10 UI band values to EqualizerProcessor as 31 native bands. */
  const applyAllBandsToNative = useCallback(async (bands: number[]) => {
    if (!isActive) return;
    const nativeGains = uiBandsToNative31(bands);
    getPlayerModule()
      ?.applyEQBands(nativeGains)
      .catch(e => console.warn('[EQ] applyEQBands failed:', e));
  }, [isActive]);

  /**
   * Push a single UI band change to EqualizerProcessor.
   * We always send the full 31-band array because the native processor
   * replaces all values atomically — partial updates aren't supported.
   */
  const applyBandToNative = useCallback(async (uiIndex: number, gain: number) => {
    ...
    getPlayerModule()
      ?.applyEQBands(nativeGains)
      .catch(e => console.warn('[EQ] applyBand failed:', e));
  ...
  useEffect(() => {
    AsyncStorage.getItem(STORAGE_KEY).then(json => {
      if (json) {
        ...
        } catch (e) { console.warn('[EQ] Load error:', e); }
      }
    }).finally(() => setIsLoading(false));
  }, []);

  // Debounced save to AsyncStorage on any state change
  useEffect(() => {
    if (isLoading) return;
  ...
  // Push restored EQ to EqualizerProcessor after load
  useEffect(() => {
    if (isLoading) return;
  ...
  const handleBassChange = useCallback(async (_knobId: string, value: number) => {
    ...
    getPlayerModule()
      ?.applyEQBands(uiBandsToNative31(newBands))
      .catch(() => {});
  }, [isActive, eqState.bands]);

  const handleTrebleChange = useCallback(async (_knobId: string, value: number) => {
  ...
  // ── EQ toggle — calls setEQEnabled on EqualizerProcessor ──────────────────
  ...
  const toggleEQ = useCallback(async () => {
    ...
    // setEQEnabled bypasses or re-enables EqualizerProcessor in the DSP chain
    getPlayerModule()?.setEQEnabled(next).catch(() => {});
  ...
  return (
    <View style={styles.root}>
      ...
      >
        ...
        <View style={styles.knobsRow}>
          <View style={styles.knobCell}>
            <RotaryKnob
              knobId="bass" value={bass} label="BASS"
              size={scale(72)} enabled={isActive} onValueChange={handleBassChange}
            />
          </View>
          ...
          <View style={styles.knobCell}>
            <RotaryKnob
              knobId="treble" value={treble} label="TREBLE"
              size={scale(72)} enabled={isActive} onValueChange={handleTrebleChange}
            />
          </View>
        </View>
      </ScrollView>
      ...
      <SavePresetModal
        ...
        preamp={0}
        onSaved={name => {
          setEqState(p => ({ ...p, selectedPreset: name }));
      ...
      <View style={modalStyles.overlay}>
        <View style={[modalStyles.sheet, { paddingBottom: insets.bottom + scale(16) }]}>
          ...
          <FlatList
            ...
            renderItem={({ item }) => (
              <TouchableOpacity
                style={[modalStyles.item, selectedPreset === item.name && modalStyles.itemActive]}
                onPress={() => onSelectPreset(item)}
              >
                <Text style={[modalStyles.itemName, selectedPreset === item.name && modalStyles.itemNameActive]}>
                  {item.name}
                </Text>
...
const SavePresetModal: React.FC<{
  ...
  insets:   { bottom: number };
}> = ({ visible, onClose, bands, preamp, onSaved, insets }) => {
    ...
    try {
      ...
      await AsyncStorage.setItem(
        ...
        JSON.stringify([
          ...
          { id: `c${Date.now()}`, name: trimmed, bands, preamp, is_factory: false },
        ]),
      );
