...
function SectionCount({ count, label }: { count: number; label: string }) {
  ...
  return <Text style={scStyles.text}>{count} {label}{count !== 1 ? "s" : ""}</Text>;
}
...
function DownloadsPanel({ sort, router }: { sort: string; router: ReturnType<typeof useRouter> }) {
  ...
  const ActiveDownloadItem = ({ song }: { song: any }) => (
    <View style={adRowStyles.row}>
      <CoverArt uri={song.thumbnail} size={moderateScale(44)} />
      <View style={adRowStyles.info}>
        <Text style={adRowStyles.title} numberOfLines={1}>{song.title}</Text>
        <View style={adRowStyles.progressTrack}>
          <View style={[adRowStyles.progressFill, { width: `${Math.round((song.progress ?? 0) * 100)}%` }]} />
        </View>
        <Text style={adRowStyles.pct}>{Math.round((song.progress ?? 0) * 100)}%</Text>
      </View>
    </View>
  );

  if (tracks.length === 0 && activeDownloads.length === 0) {
    return (
      <EmptyState
          ...
          return (
            <View style={rowStyles.row}>
              <TouchableOpacity
                ...
                onPress={() => { triggerHaptic(); playDownloadedSong(item, tracks); }}
                activeOpacity={0.7}
              >
                <CoverArt uri={item.localArtworkUri} size={moderateScale(52)} placeholder="cloud-download-outline" />
                <View style={rowStyles.info}>
                  <Text style={[rowStyles.title, isPlaying && { color: C.gold }]} numberOfLines={1}>{item.title}</Text>
                  <Text style={rowStyles.sub} numberOfLines={1}>{item.artist}</Text>
                  <SourceBadge isLocal />
...
const adRowStyles = ScaledSheet.create({
  row: {
    flexDirection: "row",
    alignItems: "center",
    paddingHorizontal: "16@s",
...
function SortRow({
  ...
  return (
    <View style={sRowStyles.wrapper}>
      <ScrollView
        horizontal
        showsHorizontalScrollIndicator={false}
        contentContainerStyle={sRowStyles.sortRow}
        style={{ flex: 1 }}
      >
        {options.map((opt) => {
          ...
          return (
            <TouchableOpacity
              key={opt}
              style={[sRowStyles.sortPill, isSel && sRowStyles.sortPillActive]}
              onPress={() => { triggerHaptic(); onSelect(opt); }}
              activeOpacity={0.7}
            >
              <Text style={[sRowStyles.sortText, isSel && sRowStyles.sortTextActive]}>{opt}</Text>
            </TouchableOpacity>
          );
        })}
      </ScrollView>
      <View style={sRowStyles.actions}>
        {onShuffle && (
          <TouchableOpacity style={sRowStyles.actionBtn} onPress={() => { triggerHaptic(); onShuffle(); }} hitSlop={8}>
            <Ionicons name="shuffle" size={moderateScale(17)} color={C.textSub} />
          </TouchableOpacity>
        )}
        {showViewToggle && onToggleView && (
          <TouchableOpacity style={sRowStyles.actionBtn} onPress={() => { triggerHaptic(); onToggleView(); }} hitSlop={8}>
            <Ionicons name={gridView ? "list" : "grid-outline"} size={moderateScale(17)} color={C.textSub} />
          </TouchableOpacity>
        )}
      </View>
    </View>
...
const sRowStyles = ScaledSheet.create({
...
export default function LibraryScreen() {
  ...
  const isFloatingPlayerVisible = !!currentTrack;
  ...
  const setActiveSort = (s: string) => setSort((prev) => ({ ...prev, [activeTab]: s }));
    ...
    <View style={[lStyles.container, { paddingTop: top }]}>
      ...
      <View style={lStyles.pageHeader}>
        ...
        <View style={lStyles.headerActions}>
          <TouchableOpacity style={lStyles.headerBtn} onPress={() => { triggerHaptic(); setSearchFocused((v) => !v); }} hitSlop={10}>
            <Ionicons name="search" size={moderateScale(20)} color={C.goldShimmer} />
          </TouchableOpacity>
          <TouchableOpacity style={lStyles.headerBtn} onPress={() => { triggerHaptic(); router.push("/(modals)/settings"); }} hitSlop={10}>
      ...
      >
        ...
        {/* Tab row */}
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={lStyles.tabRow}>
          {TABS.map((tab) => {
