/**
 * ArtistPageScreen — v7  (YouTube-style channel page)
 *
  ...
  if (error || !meta) {
    return (
      <View style={localStyles.centered}>
        ...
        <Text style={localStyles.errorText}>{error ?? "Artist not found."}</Text>
        <TouchableOpacity style={localStyles.backBtn} onPress={() => router.back()}>
    ...
    >
      ...
      <View style={localStyles.bannerContainer}>
        {meta.bannerUrl ? (
          ...
          <View style={[localStyles.banner, { backgroundColor: "#1a1a1a" }]} />
        )}
        <LinearGradient
      ...
      <View style={localStyles.headerRow}>
        <View style={localStyles.avatarWrapper}>
          {meta.avatarUrl ? (
            ...
            <View style={[localStyles.avatar, { backgroundColor: "#333" }]} />
          )}
        </View>

        <View style={localStyles.nameBlock}>
          <View style={{ flexDirection: "row", alignItems: "center", gap: 6 }}>
            <Text style={localStyles.artistName} numberOfLines={1}>{meta.name}</Text>
            {meta.isVerified && (
              <MaterialIcons name="verified" size={moderateScale(18)} color="#3ea6ff" />
            )}
          </View>
          {!!meta.subscriberCount && (
            <Text style={localStyles.subscriberText}>{formatSubscribers(meta.subscriberCount)}</Text>
          )}
        </View>
      </View>
...
const tabBarStyles = StyleSheet.create({
  ...
  pillActive:  { backgroundColor: GOLD_PRIMARY },
