// app/(tabs)/search/album.tsx
/**
 * AlbumPageScreen - expo-av version
 *
...
const TypedFlashList = FlashList as React.ComponentType<
  FlashListPropsWithEstimated<AlbumSong>
>;
...
interface AlbumSong {
  ...
  thumbnail:        string;
  url:              string;
  /** Pre-formatted display string e.g. "3:45" — kept for UI rendering */
  duration:         string;
  /** Raw duration in seconds from the API — used when building Song for player */
...
// Normalise AlbumSong → types/song.Song for the player.
// AlbumSong.duration is a display string ("3:45"); Song.duration is number (seconds).
// AlbumSong.durationSeconds carries the raw value for this conversion.
const toPlayerSong = (s: AlbumSong): Song => ({
  id:        s.id,
  title:     s.title,
  artist:    s.artist,
  ...
  videoId:   undefined,
  videoUrl:  undefined,
  duration:  s.durationSeconds,          // number | undefined — matches Song.duration?
});
...
export default function AlbumPageScreen() {
  ...
  const ListHeader = () => (
    <>
      ...
      <Text style={styles.subtitleText}>{albumData?.subtitle}</Text>
      <Text style={styles.subtitleText}>{albumData?.second_subtitle}</Text>
    </>
  );

  const renderSongItem = ({ item, index }: { item: AlbumSong; index: number }) => (
    <View key={item.id + index} style={styles.songItem}>
      ...
      >
        ...
        <View style={styles.resultText}>
          ...
          <Text style={styles.resultArtist} numberOfLines={1}>{item.duration}</Text>
        </View>
      </TouchableOpacity>
        ...
        {(albumData?.songs?.length ?? 0) > 0 && (
          <FAB
            ...
            onPress={async () => {
              ...
              await playPlaylist(albumData.songs.map(toPlayerSong));
...
const styles = ScaledSheet.create({
  ...
  headerText: {
    fontSize: "20@ms", fontWeight: "bold",
    color: Colors.text, textAlign: "left", width: "82%",
  },
  ...
  titleText: {
    fontSize: "24@ms", fontWeight: "bold",
    color: Colors.text, marginHorizontal: 15,
    textAlign: "center", marginBottom: 5,
  },
  subtitleText: {
    color: Colors.text, textAlign: "center",
    fontSize: "15@ms", marginBottom: 5,
  },
  songItem: {
